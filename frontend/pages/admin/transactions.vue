  ...
  <div class="space-y-8 animate-in fade-in slide-in-from-right-4 duration-500">
    ...
    <div class="flex flex-col md:flex-row gap-4 items-center bg-card p-4 rounded-2xl shadow-sm border">
      <div class="relative flex-1 w-full">
        ...
        <UiInput placeholder="Tìm mã giao dịch, tên user..." class="pl-10 h-11 bg-muted/30 border-0 rounded-xl" />
      </div>
    ...
    <UiCard class="border-0 shadow-md overflow-hidden">
      <UiTable>
        <UiTableHeader class="bg-indigo-50 dark:bg-slate-900 font-black">
          <UiTableRow>
            <UiTableHead>Giao dịch</UiTableHead>
            <UiTableHead>Người thực hiện</UiTableHead>
            <UiTableHead>Số tiền</UiTableHead>
            <UiTableHead>Phương thức</UiTableHead>
            <UiTableHead>Thời gian</UiTableHead>
            <UiTableHead class="text-right">Trạng thái</UiTableHead>
          </UiTableRow>
        </UiTableHeader>
        <UiTableBody>
          <UiTableRow v-for="t in transactions" :key="t.id" class="hover:bg-muted/5 transition-colors border-b last:border-0">
            <UiTableCell>
            ...
            <UiTableCell class="font-bold text-indigo-600">@{{ t.user }}</UiTableCell>
            <UiTableCell class="font-black text-sm">{{ t.amount }}</UiTableCell>
            <UiTableCell class="text-xs font-medium">{{ t.method }}</UiTableCell>
            <UiTableCell class="text-[10px] font-bold text-muted-foreground">{{ t.time }}</UiTableCell>
