...
const stats = [
  { name: 'Phí sàn thu được', value: '150,250,000đ', change: '+15.2%', icon: CreditCard, color: 'text-indigo-500', bg: 'bg-indigo-500/10' },
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div>
        <h1 class="text-3xl font-black tracking-tight flex items-center gap-3">
          Admin Dashboard <div class="h-2 w-2 rounded-full bg-indigo-600 animate-pulse"></div>
        </h1>
        ...
        <div class="relative z-10">
          <div class="flex justify-between items-center mb-10">
             <h2 class="font-bold text-xl flex items-center gap-3">
               <Globe class="h-6 w-6 text-indigo-400" /> Lưu lượng hệ thống (Real-time)
             </h2>
             <div class="flex gap-4">
               <div class="text-right">
                 <p class="text-[10px] uppercase font-bold text-indigo-400">Đang Online</p>
                 <p class="text-lg font-black tracking-tighter">842</p>
               </div>
               <div class="text-right border-l border-white/10 pl-4">
                 <p class="text-[10px] uppercase font-bold text-indigo-400">Yêu cầu/giây</p>
          ...
          <div class="h-40 flex items-end gap-1 px-2">
            <div v-for="(h, i) in [20, 35, 45, 30, 60, 80, 75, 40, 50, 65, 85, 95, 70, 55, 40, 30, 25]" :key="i" 
                 class="flex-1 bg-indigo-500/40 rounded-t-sm transition-all duration-700 hover:bg-indigo-400"
                 :style="{ height: h + '%' }">
            </div>
          </div>
          <div class="flex justify-between text-[10px] uppercase font-black text-indigo-400 mt-6 px-2 tracking-widest">
            <span>00:00</span>
            <span>06:00</span>
            <span>12:00</span>
            <span>18:00</span>
      ...
      <UiCard class="p-6 border-0 shadow-md space-y-6 flex flex-col">
        ...
        <div class="space-y-6 flex-1">
          <div v-for="log in recentActions" :key="log.time" class="flex items-start gap-4 relative">
            ...
            <div class="h-8 w-8 rounded-full bg-indigo-50 flex items-center justify-center shrink-0 border border-indigo-100 relative z-10">
              <ShieldCheck class="h-4 w-4 text-indigo-600" />
            </div>
            <div class="flex-1 min-w-0">
              <p class="text-sm font-bold text-slate-800 dark:text-slate-100">{{ log.user }}</p>
              <p class="text-xs text-muted-foreground mt-0.5">{{ log.action }}: <span class="font-medium text-indigo-600">{{ log.target }}</span></p>
