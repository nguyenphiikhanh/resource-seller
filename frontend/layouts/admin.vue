...
<template>
  <div class="flex min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100">
    ...
    <aside class="hidden w-64 border-r bg-background lg:block shadow-xl shadow-indigo-500/5">
      <div class="flex h-16 items-center border-b px-6">
        <NuxtLink to="/" class="flex items-center gap-2 font-black text-xl tracking-tighter">
          ...
          <span>MMO <span class="text-indigo-600">Admin</span></span>
        </NuxtLink>
      </div>
      <div class="flex flex-col gap-1 p-4">
        ...
        <NuxtLink 
          ...
          :key="item.name" 
          :to="item.href"
          class="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-bold transition-all hover:bg-indigo-50 hover:text-indigo-600 group"
          active-class="bg-indigo-600/10 text-indigo-600 shadow-sm"
        >
    ...
    <UiSheet v-model:open="isSidebarOpen">
      <UiSheetContent side="left" class="w-64 p-0 border-0">
      ...
      <header class="sticky top-0 z-30 flex h-16 items-center justify-between border-b bg-background/80 px-4 backdrop-blur-md lg:px-8 border-indigo-100">
        <div class="flex items-center gap-4">
          ...
          <div class="relative hidden max-w-md items-center md:flex group">
            <Search class="absolute left-3.5 h-4 w-4 text-muted-foreground group-focus-within:text-indigo-600 transition-colors" />
            <UiInput 
          ...
          <UiDropdownMenu>
            <UiDropdownMenuTrigger as-child>
              <button class="flex items-center gap-2 rounded-xl bg-card border px-2 py-1.5 hover:bg-muted transition-all shadow-sm border-indigo-100">
                <UiAvatar class="h-8 w-8 border-2 border-indigo-600/20">
                  <UiAvatarFallback class="bg-indigo-600 text-white font-black text-xs">{{ user.avatar }}</UiAvatarFallback>
                </UiAvatar>
            ...
            <UiDropdownMenuContent align="end" class="w-56 rounded-2xl p-2 shadow-2xl border-indigo-50">
              <UiDropdownMenuLabel class="font-black text-xs uppercase tracking-widest text-muted-foreground p-3">Quản trị viên</UiDropdownMenuLabel>
